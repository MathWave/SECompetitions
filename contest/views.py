...
def task_settings(request):
    try:
        ...
        context = {'block': block_name, 'task': this_task[1], 'legend': this_task[3], 'input': this_task[4],
                   ...
                   'tests': forms.TestsForm(), 'block_id': this_task[2], 'task_id': task_id, 'time_limit': this_task[7],
                   'is_superuser': check_teacher(request)}
        if request.method == 'POST':
            ...
            cursor.execute("UPDATE Tasks SET "
                           ...
                           (request.POST['legend'],
                            ...
                            int(request.POST['time_limit']) if is_integer(request.POST['time_limit']) else 10000,
                            request.GET['task_id']))
        ...
        if request.method == 'GET':
            ...
            if 'file' in request.FILES.keys():
                ...
                try:
                    ...
                    connector, cursor = open_db()
                    cursor.execute('UPDATE Solutions SET result = "TEST ERROR" WHERE id = ?', (index,))
                    close_db(connector)
                    flag = False
                remove(solution_dir + 'solution.zip')
                if flag:
                    from contest.TesterGlobal import TesterGlobal
                    TesterGlobal(index, this_task[0], this_task[1], request.user.username, this_task[7]).start()
